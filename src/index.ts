...
function formatArgsCompact(args: Record<string, unknown>): string {
...
function formatDataAsReadableText(data: Record<string, unknown>, indent = 0): string {
  const lines: string[] = [];
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    if (isRecord(value)) {
      ...
      lines.push(formatDataAsReadableText(value, indent + 1));
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        if (isRecord(item)) {
          lines.push(`${indentStr}  [${i + 1}]`);
          lines.push(formatDataAsReadableText(item, indent + 2));
        } else {
          lines.push(`${indentStr}  ${i + 1}. ${JSON.stringify(item)}`);
        }
      }
    } else {
...
/**
 * Encodes tool responses in LLM-optimized markdown format.
 *
 * This function converts structured JSON responses into human-readable markdown,
 * providing significant token efficiency improvements (30-40% reduction) while
 * maintaining machine parseability.
 *
 * BEFORE (JSON format - ~450 tokens):
 * ```json
 * {
 *   "summary": "Found 5 messages in INBOX. Showing 5 starting at 1.",
 *   "data": {
 *     "account_id": "default",
 *     "mailbox": "INBOX",
 *     "total": 5,
 *     "messages": [{
 *       "message_id": "default:INBOX:1234567890:1",
 *       "mailbox": "INBOX",
 ...
 *   }],
 *   "_meta": {
 *     "now_utc": "2024-01-15T10:30:00Z",
 *     "security_note": "Untrusted email content",
 *     "read_side_effects": "none"
 *   }
 * }
 * ```
 *
 * AFTER (Markdown format - ~280 tokens, 38% reduction):
 * ```markdown
 * ## Found 5 messages in INBOX. Showing 5 starting at 1.
 *
 * **Result:**
 * account_id: "default"
 * mailbox: "INBOX"
 ...
 *
 * **Next actions:**
 * 1. mail_imap_get_message [account_id="default" message_id="default:INBOX:1234567890:1"] — Fetch full details for the first message
 *
 * *now_utc="2024-01-15T10:30:00Z" security_note="Untrusted email content" read_side_effects="none"*
 * ```
 *
 * Benefits:
 * - Natural language processing: LLMs can parse markdown more efficiently
 * - Token efficiency: 30-40% reduction in token usage
 * - Better readability: Clear hierarchy and actionable suggestions
 * - Maintained structure: Key data remains organized and accessible
 */
function encodeToolResponseText(value: ToolJsonResponse): string {
  ...
  // Data section - flat representation for key data
  if (value.data) {
    parts.push('**Result:**');
    parts.push(formatDataAsReadableText(value.data as Record<string, unknown>));
  }

  // Error section
  if (value.error) {
    parts.push(`❌ Error: ${value.error.message}`);
  }

  // Hints section - minimal format
  if (value.hints && value.hints.length > 0) {
    parts.push('\n**Next actions:**');
    value.hints.forEach((hint, i) => {
      const argsStr = hint.arguments ? ` ${formatArgsCompact(hint.arguments)}` : '';
      parts.push(`${i + 1}. ${hint.tool}${argsStr} — ${hint.reason}`);
    });
  }

  // Metadata - compact format
  if (value._meta) {
    const metaStr = Object.entries(value._meta)
      .map(([k, v]) => `${k}=${JSON.stringify(v)}`)
      .join(' ');
    parts.push(`\n*${metaStr}*`);
  }
